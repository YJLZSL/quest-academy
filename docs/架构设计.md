# 问学 Quest Academy 架构设计文档

> 本文档描述问学 Quest Academy 的整体架构、分层职责、数据流、AI 调用流程、路由结构与安全架构。
>
> **技术栈**：Flutter 3.44.4 / Dart 3.12.2，支持 Android + Windows 双端
>
> **最后更新**：2026-08-25（v0.6.0 同步：品牌改名 问学 Quest Academy、移除吉祥物与 Rive）
...
问学 Quest Academy 采用**分层架构**，从上到下分为五层，每层职责清晰、依赖方向单向向下：
...
│   └── theme/                    #   主题（app_theme.dart, quest_colors.dart, shape_variants.dart）
├── data/                         # 数据层
│   ├── db/                       #   Drift 数据库
│   │   ├── database.dart         #     表定义 + QuestDatabase
...
└── shared/                       # 共享层（跨 feature 复用）
    ...
    └── widgets/                  #   通用组件（quest_card, quest_button 等）
```
...
问学 Quest Academy 采用**单向数据流**，从用户操作到 UI 更新的完整路径如下：
...
AI 调用是问学 Quest Academy 的核心流程，涉及提示词注入、Provider 选择、流式解析、安全脱敏多个环节。
...
安全是问学 Quest Academy 的核心设计原则。API Key 的存储、传输、日志全链路均有安全措施。
